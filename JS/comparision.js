...
function renderizarResultado() {
    ...
    // ===== GERAR TABELA HTML =====
    let tabelaHTML = '<table class="w-full border-collapse"><thead><tr class="bg-gray-100">';
    tabelaHTML += '<th class="border border-gray-300 px-4 py-3 text-left text-sm font-bold">Faixa Etária</th>';
    tabelaHTML += '<th class="border border-gray-300 px-4 py-3 text-center text-sm font-bold">Qtd</th>';
    
    resultados.forEach(r => {
        tabelaHTML += `<th class="border border-gray-300 px-4 py-3 text-center text-sm font-bold">${r.plano}</th>`;
    });
    ...
    // ===== LINHAS DE FAIXAS =====
    faixas.forEach(f => {
        tabelaHTML += '<tr class="hover:bg-gray-50">';
        tabelaHTML += `<td class="border border-gray-300 px-4 py-2 text-sm font-semibold">${f.nome}</td>`;
        tabelaHTML += `<td class="border border-gray-300 px-4 py-2 text-center text-sm">${f.qtd}</td>`;
        
        resultados.forEach(r => {
            const valorUnitario = obterValorPlano(tipo, r.plano, f.chave);
            tabelaHTML += `<td class="border border-gray-300 px-4 py-2 text-center text-sm">${UTILS.formatarMoeda(valorUnitario)}</td>`;
        });
    ...
    // ===== LINHA DE SUBTOTAL =====
    tabelaHTML += '<tr class="bg-blue-50 font-bold">';
    tabelaHTML += '<td class="border border-gray-300 px-4 py-3 text-sm" colspan="2">SUBTOTAL</td>';
    
    resultados.forEach(r => {
        tabelaHTML += `<td class="border border-gray-300 px-4 py-3 text-center text-sm">${UTILS.formatarMoeda(r.subtotal)}</td>`;
    });
    
    tabelaHTML += '</tr>';
    
    // ===== LINHA DE TOTAL =====
    tabelaHTML += '<tr class="bg-oeste-turqueza text-white font-bold">';
    tabelaHTML += '<td class="border border-gray-300 px-4 py-3 text-sm" colspan="2">VALOR TOTAL</td>';
    
    resultados.forEach(r => {
        const isMelhor = r.valorFinal === menorValor;
        const classeMelhor = isMelhor ? 'bg-green-500' : '';
        
        tabelaHTML += `<td class="border border-gray-300 px-4 py-3 text-center text-sm ${classeMelhor}">
            ${UTILS.formatarMoeda(r.valorFinal)}
            ${isMelhor ? '<br><span class="text-xs">⭐ MELHOR</span>' : ''}
        </td>`;
    });
    ...
    tabelaHTML += `
        <div class="mt-4 p-3 bg-yellow-50 border-2 border-yellow-300 rounded-lg text-center">
            <p class="text-sm font-bold text-yellow-800">
                <i class="fas fa-star text-yellow-500"></i>
                Melhor Custo-Benefício: ${UTILS.formatarMoeda(menorValor)}
            </p>
        </div>
    `;
